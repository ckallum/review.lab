...
- <e.g., "The null check is handled by the ORM's find method which throws NotFound, not nil">
...
Use Tier 2 when escalation detection (below) identifies a prior reply on the same thread. Include maximum evidence to close the discussion.
...
Before composing a reply, check if a prior reply already exists on this comment thread:

1. **For line-level comments:** Fetch replies via `gh api repos/$REPO/pulls/$PR_NUMBER/comments/$COMMENT_ID/replies`. Check if any reply body contains markers: `**Fixed**`, `**Not a bug.**`, `**Already fixed**`.

2. **For top-level comments:** Scan the fetched issue comments for replies posted after the Greptile comment that contain those markers.

3. **If a prior reply exists AND Greptile posted again on the same file+category:** Use Tier 2 (firm) templates.

4. **If no prior reply exists:** Use Tier 1 (friendly) templates.
...
- `$HOME/.claude/review/projects/$REMOTE_SLUG/greptile-history.md` (per-project)
- `$HOME/.claude/review/greptile-global.md` (global aggregate)
...
Example entries:
```text
2026-03-15 | myorg/myapp | fp | src/services/auth.ts | race-condition
2026-03-15 | myorg/myapp | fix | src/models/user.ts | null-check
2026-03-15 | myorg/myapp | already-fixed | lib/payments.ts | error-handling
```
