...
- [ ] **T1.7 — Demo HTML extraction.** Pull `~/Documents/Claude/Projects/review.dev/review-dev-demo.html`, strip tab nav and concepts 02–06, replace numeric `conf 0.94` with word bands. Land at `web/index.html`. Parameterise every hardcoded string to fetch from `/api/pr/:id/rev/:n`.
