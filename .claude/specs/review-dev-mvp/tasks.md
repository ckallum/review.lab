...
- [x] **T1.3 — `reviewdev serve` skeleton.** Hono server with port probe (7891–7899), writes chosen port to `<repo>/.reviewdev/port`. `GET /health` returns `{ok: true, port: N, schema_version: N}`. Foreground process, structured logs to stdout. Server binds `localhost` only; migrations apply forward-only on start (reuses T1.2 `openDb`/`applyMigrations`); `schema_version` reads `MAX(version)` from `meta`.
