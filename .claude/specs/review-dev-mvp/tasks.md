...
- [x] **T1.8 — File-based chapter fallback.** Group hunks by top-level directory, secondary by extension. ~~Emit 3–7 chapters with marker numbering.~~ No LLM. Used when `ANTHROPIC_API_KEY` is unset.
  - **Addendum (2026-07-25):** the code guarantees **1–7** chapters, not 3–7. 7 is a hard cap (the smallest groups fold into an `(other)` bucket); 3 is a best-effort target the extension split aims at; a diff spanning fewer than 3 (dir, ext) cells honestly yields fewer (a 1-file PR → 1 chapter) rather than fabricating empty ones. The secondary extension split is **conditional** — applied only when there are fewer than 3 top-level directories. Markers are `§ 01`…. The `ANTHROPIC_API_KEY not set — using file-based chapters` publish message (SPEC.md § LLM config) is deferred to T2.1, where the key check first exists.
